...
2. The interactive nature of both the *Authorization Code Grant* flow (i.e. most
   commonly requiring a browser redirection and user input to obtain a token)
   and the *Client Credentials Grant* flow where the client has to authenticate
   when making a token issuer request to a server's token endpoint.
...
The `artifacts.lock.yaml` schema is extended with an optional `auth` per a
single URL resource as follows:
...
| `value` | string | Yes | HTTP Header value using env variable placeholders and custom strings |
...
1. Resolve the `auth` configuration maps for each lockfiles artifact in the
   generic backend
2. Read the corresponding environment variables to extract the values based
   ...
   field's content
3. Inject headers into the `aiohttp.ClientSession` context


## Usage

The full end-users bearer authentication workflow might look like this:
...
However, AWS Signature V4 is significantly more complex than header injection—it
